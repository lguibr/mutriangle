# File: mutriangle/training/worker_manager.py
import logging
...
class WorkerManager:
    ...
    def __init__(self, components: "TrainingComponents"):
        ...
        # Get Trieye actor name and run base dir string via remote calls
        # These calls happen *after* the actor is confirmed ready in setup.py
        try:
            ...
            logger.info(
                f"WorkerManager obtained Trieye info: Name='{self.trieye_actor_name}', RunDir='{self.run_base_dir_str}'"
            )
        except Exception as e:
            logger.error(f"Failed to get info from TrieyeActor: {e}", exc_info=True)
            raise RuntimeError(
                "Could not communicate with TrieyeActor for setup."
            ) from e
    ...
    def initialize_workers(self):
        ...
        # Use the run base dir obtained from the Trieye actor
            ...
            try:
                ...
                worker = SelfPlayWorker.options(
                    ...
                    * 1024,  # 300MB per worker (helps Ray prevent OOM)
                    ...
                    run_base_dir=run_base_dir_for_worker,  # Pass run dir string
        ...
        # Note: Don't free initial weights_ref here - workers need it during async init
        # Workers will retrieve weights asynchronously, freeing too early causes ObjectFreedError
        # Cleanup happens during periodic updates and final shutdown
    ...
    def update_worker_networks(self, global_step: int):
        ...
        if not all_tasks:
            # No tasks to wait for, safe to free immediately
            try:
                ray.internal.free([weights_ref])
            except Exception as e:
                logger.warning(f"Failed to free weights from object store: {e}")
            return
        try:
            # Wait for all workers to acknowledge receipt before freeing
            ...
            # Free weights AFTER workers have confirmed receipt via ray.get()
            try:
                ray.internal.free([weights_ref])
            except Exception as e:
                logger.warning(f"Failed to free weights from object store: {e}")
