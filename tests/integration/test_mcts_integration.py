...
def test_mutrimcts_returns_valid_data():
    ...
    assert isinstance(root_value, (float, int)), (
        f"root_value should be float/int, got {type(root_value)}"
    )
    ...
    assert len(mcts_policy) > 0, (
        f"mcts_policy should NOT be empty! Got length {len(mcts_policy)}"
    )
    ...
    assert network.call_count > 1, (
        f"Network should be called multiple times, got {network.call_count}"
    )

    print(
        f"✅ MCTS OK: {len(visit_counts)} actions, {total_visits} visits, policy_sum={policy_sum:.4f}"
    )
