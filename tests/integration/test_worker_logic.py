...
def test_episode_logic_with_real_mcts():
    ...
    for i, policy in enumerate(mcts_policies):
        ...
        assert abs(policy_sum - 1.0) < 0.01, f"Policy {i} sum={policy_sum:.4f}"
    ...
    print(
        f"   First policy: {len(mcts_policies[0])} actions, sum={sum(mcts_policies[0].values()):.4f}"
    )
    print(
        f"   Total visits first step: {sum([1 for k in visit_counts])} unique actions"
    )
